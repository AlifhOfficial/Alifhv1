...
export function ListingsTabs({ 
  ...
  return (
    <div className="flex flex-col compact:flex-row items-start compact:items-center justify-between gap-3 compact:gap-4">
      {/* Main Tabs */}
      <div className="flex items-center gap-4 compact:gap-6 overflow-x-auto scrollbar-hide w-full compact:w-auto pb-1 compact:pb-0">
        {mainTabs
      ...
      {deepInventoryTotal > 0 && (
        <div className="relative shrink-0">
          ...
          {/* Dropdown Menu */}
          {isDeepOpen && (
            <div className="absolute left-0 compact:left-auto compact:right-0 top-full mt-2 w-48 py-2 rounded-xl bg-sidebar border border-sidebar-border shadow-lg z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              {/* All option */}
              <button
                onClick={() => {

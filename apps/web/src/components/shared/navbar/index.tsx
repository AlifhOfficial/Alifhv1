...
export function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-40 h-14 regular:h-16 bg-background will-change-transform [transform:translateZ(0)] [backface-visibility:hidden]">
        <div className="flex h-full items-center max-w-[1600px] mx-auto px-4 regular:px-6 large:px-8">
            <div className="flex min-w-0 flex-1 items-baseline">
              ...
              {/* Desktop Navigation */}
              <div className="hidden regular:flex items-baseline gap-0.5 large:gap-1">
                {navItems.map((item) => (
                  <div 
                    key={item.label}
                  ...
                  >
                    <Link
                      href={item.href}
                      className={`inline-flex items-baseline rounded-md px-2.5 large:px-4 py-2.5 text-subhead font-semibold leading-none tracking-tight transition-colors ${
                        pathname === item.href
                          ? "text-foreground"
            ...
            <div className="ml-auto flex flex-shrink-0 items-center gap-2">
              ...
              <div className="relative" data-theme-menu-container>
                <button
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-border/60 bg-muted/30 text-muted-foreground transition-colors hover:bg-background/80 hover:text-foreground"
              ...
              {mounted && isAuthenticated && user?.id && (
                <>
                  ...
                  <div className="regular:hidden flex h-10 items-center rounded-full border border-border/60 bg-muted/30 px-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    <ProfileMenu
                      user={user}
                      showMenu={showProfileMenu}
                      onToggleMenu={(e) => {
              ...
              {/* Signed-out / pre-hydration profile actions */}
              {(!mounted || !isAuthenticated || !user?.id) && (
                <div className="flex h-10 items-center rounded-full border border-border/60 bg-muted/30 px-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                  <ProfileMenu
                    user={user}
                    showMenu={showProfileMenu}
                    onToggleMenu={(e) => {

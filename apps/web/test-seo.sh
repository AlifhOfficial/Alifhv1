...
curl -s "http://localhost:3000/listings?make=Toyota&emirate=dubai" | grep -q 'name="robots" content="noindex' && echo "noindex ✓" || echo "FAIL"
echo -n "✓ Price ranges: "
curl -s "http://localhost:3000/listings?minPrice=50000&maxPrice=100000" | grep -q 'name="robots" content="noindex' && echo "noindex ✓" || echo "FAIL"
echo -n "✓ Pagination: "
curl -s "http://localhost:3000/listings?page=2" | grep -q 'name="robots" content="noindex' && echo "noindex ✓" || echo "FAIL"
echo -n "✓ Base /listings (should be indexable): "
curl -s "http://localhost:3000/listings" | grep -q 'name="robots"' && echo "FAIL (has noindex)" || echo "indexable ✓"

echo ""
echo "🗺️  Testing Sitemap..."
echo -n "✓ Sitemap accessible: "
curl -s "http://localhost:3000/sitemap.xml" | grep -q '<urlset' && echo "200 ✓" || echo "FAIL"
echo -n "✓ Contains Black listings: "
curl -s "http://localhost:3000/sitemap.xml" | grep -q 'black-listings' && echo "found ✓" || echo "FAIL"
echo -n "✓ Contains regional specs: "
curl -s "http://localhost:3000/sitemap.xml" | grep -q 'gcc-specs' && echo "found ✓" || echo "FAIL"

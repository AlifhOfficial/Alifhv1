...
export default function InventoryArchiveScreen() {
  ...
  async function handleConfirm() {
    ...
    try {
      await toggleArchiveListing(listingId, !isArchived);
      await queryClient.invalidateQueries({ queryKey: ['inventory'] });
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      router.back();
    } catch (err) {
      const message = err instanceof Error ? err.message : `Failed to ${actionLabel.toLowerCase()} listing`;
      setError(message);
      ...
      <View style={styles.actions}>
        ...
        <HapticPressable
          ...
          style={[
            ...
            { backgroundColor: isArchived ? colors.primary : colors.warning, opacity: loading ? 0.7 : 1 },
          ]}
        >
          {loading ? (

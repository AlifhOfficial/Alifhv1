 ...
 *
 * Mobile-local copy of the shared schema to avoid importing database workspace
 * source during mobile bundling. Keep in sync with
 * packages/database/src/schema/search-types.ts.
 *
 * @module apps/mobile/lib/search-types
 */
...
export function urlToSearchParams(urlParams: URLSearchParams): SearchParams {
  ...
  return {
    ...
    emirate: parseArray('emirate') as SearchParams['emirate'],

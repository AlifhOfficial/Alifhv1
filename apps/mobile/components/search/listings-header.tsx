/**
 * ListingsHeader - Mobile
 * 
 * Revolut-inspired dynamic island header
 * Premium mobile-first design with buttery smooth animations
 */

import { useColor } from '@/hooks/useColor';
import { FONT_FAMILY, FONT_FAMILY_MEDIUM, FONT_FAMILY_SEMIBOLD, FONT_FAMILY_BOLD } from '@/theme/globals';
import { Search, SlidersHorizontal, ArrowUpDown, X, ChevronRight } from 'lucide-react-native';
  ...
  return (
    <View style={[styles.container, { paddingTop: topInset }]}>
          ...
          {searchExpanded ? (
            ...
            <Pressable onPress={expandSearch} style={styles.searchCollapsed}>
              <View style={[styles.searchPill, { backgroundColor: accent }]}>
                ...
                <Text 
                  style={[styles.searchPlaceholder, { color: mutedFg }]}
                  numberOfLines={1}
                >
                  {params.q || 'Search cars...'}
                </Text>
                {params.q && (
                  <ChevronRight size={14} color={mutedFg} strokeWidth={1.5} />
                )}
              </View>
            </Pressable>
          )}
      ...
      <QuickFilters
        ...
        facets={facets}
        isLoading={isLoading}
        onSelectMake={(make) => onFiltersChange({ make: [make], model: undefined, trim: undefined })}
        onSelectModel={(model) => onFiltersChange({ model: [model], trim: undefined })}
        onSelectTrim={(trim) => onFiltersChange({ trim: [trim] })}
      />
...
export function ListingsHeaderSkeleton({ topInset = 0 }: { topInset?: number }) {
  const card = useColor('card');
  const accent = useColor('accent');
  const border = useColor('border');

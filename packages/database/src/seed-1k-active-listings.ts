/**
 * Seed Script: 1000 Active Car Listings
 * 
 * Creates 1000 ACTIVE listings (moderation_status=approved, lifecycle_status=active)
 * Uses existing users and partners from the database
 * Uses CDN images from https://cdn.revvup.ae/static/Black_cars/
 * 
 * MAX 3 TAGS per listing as per business rules
 * 
 * Usage: DATABASE_URL="postgresql://..." bun run src/seed-1k-active-listings.ts
 */
...
import {
  ...
  LISTING_TAGS,
  UAE_EMIRATES,
  MAX_LISTING_TAGS, // Use constant from listing-constants
  type BodyType,
  type FuelType,
...
// ===== CDN IMAGE PATHS =====
// Full CDN URLs for Black_cars images
...
function generateActiveListing(
  ...
  // Seller type based on ownership
  const sellerType = partnerId ? 'dealer' as const : 'private' as const;
  const postedByRole: ListingPostedByRole = (sellerType === 'dealer') ? 'staff' : 'user';
  
  // ALL LISTINGS ARE ACTIVE & APPROVED
  ...
  // TAGS - MAX 3 per listing (enforced)
  ...
  // Engagement metrics (active listings have views)
  ...
  // Timestamps - all listings are published (within last 7 days to avoid cron expiry)
  ...
  const originalPublishedAt = publishedAt; // First publish
  const approvedAt = new Date(publishedAt.getTime() - randomInt(1, 6) * 60 * 60 * 1000);
  const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 days from NOW (not publishedAt)
  ...
  // Partner brand info
  const partnerBrandName = sellerType === 'dealer' ? getRandomItem(['Luxury Motors Dubai', 'Premium Auto Trading', 'Elite Motors Abu Dhabi', 'Al Futtaim Motors', 'Arabian Automobiles']) : undefined;
  ...
  return {
    ...
    lifecycleStatus,
    sellerType,
    isConsignment: getRandomBoolean(0.1),
    ...
    // Badges & Tags (MAX 3 tags enforced)
    ...
    // Reservation & Sale - all null for active listings
    reservedAt: null,
    reservedBy: null,
    soldAt: null,
    soldTo: null,
...
// ===== MAIN SEED FUNCTION =====
async function seed1kActiveListings() {
  console.log('\n' + '='.repeat(60));
  console.log('🚗 SEED 1000 ACTIVE LISTINGS');
  ...
  try {
    ...
    if (partners.length === 0) {
      console.error('❌ No partners found! Please add partners first.');
      process.exit(1);
    }
    ...
    console.log(`   • Standard tier: ${standardPartners.length}\n`);
    
    // Get users for private listings
    const users = await db.select().from(user).limit(50);
    console.log(`✅ Found ${users.length} users\n`);
    
    // Generate 1000 ACTIVE listings
    const listings = [];
    const totalListings = 1000;
    
    console.log(`📝 Generating ${totalListings} ACTIVE listings...\n`);
    ...
    for (let i = 0; i < totalListings; i++) {
      ...
      // 15% private listings from users
      const isPrivate = getRandomBoolean(0.15);
      
      if (isPrivate && users.length > 0) {
        // Private listing - user-owned
        selectedUserId = getRandomItem(users).id;
        selectedPartnerId = null;
      } else {
        // Dealer listing - distribute across partner tiers
        if (blackPartners.length > 0 && getRandomBoolean(0.15)) {
          selectedPartnerId = getRandomItem(blackPartners).id;
          isBlkListing = true;
        } else if (platinumPartners.length > 0 && getRandomBoolean(0.3)) {
          selectedPartnerId = getRandomItem(platinumPartners).id;
        } else if (goldPartners.length > 0 && getRandomBoolean(0.3)) {
          selectedPartnerId = getRandomItem(goldPartners).id;
        } else if (standardPartners.length > 0) {
          selectedPartnerId = getRandomItem(standardPartners).id;
        } else {
          selectedPartnerId = getRandomItem(partners).id;
        }
      }
      
      const listing = generateActiveListing(
        selectedPartnerId,
        selectedUserId,
        isBlkListing,
      );
    ...
    console.log(`\n✅ Generated ${listings.length} ACTIVE listings\n`);
    ...
    console.log(`   Dealer listings: ${listings.filter(l => l.sellerType === 'dealer').length}`);
    console.log(`\n   Max tags per listing: ${MAX_LISTING_TAGS}`);

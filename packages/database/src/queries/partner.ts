 ...
 * Partner Queries
 *
 * CRUD operations for Partner, PartnerStaff, PartnerReview, PartnerRequest, and AuditLog tables.
 * Following the same pattern as profile queries for consistency.
 */
...
const STAFF_ID_PREFIX = 'staff_';
const REVIEW_ID_PREFIX = 'review_';
const REQUEST_ID_PREFIX = 'preq_';
const AUDIT_ID_PREFIX = 'audit_';

const makePartnerId = () => `${PARTNER_ID_PREFIX}${createId()}`;
const makeStaffId = () => `${STAFF_ID_PREFIX}${createId()}`;
const makeReviewId = () => `${REVIEW_ID_PREFIX}${createId()}`;
const makeRequestId = () => `${REQUEST_ID_PREFIX}${createId()}`;
const makeAuditId = () => `${AUDIT_ID_PREFIX}${createId()}`;
...
export type PartnerStaffRecord = typeof partnerStaff.$inferSelect;
export type PartnerStaffInsert = typeof partnerStaff.$inferInsert;
export type PartnerStaffUpdate = Partial<Omit<PartnerStaffInsert, 'id' | 'partnerId' | 'userId' | 'createdAt'>>;

export type PartnerReviewRecord = typeof partnerReview.$inferSelect;
export type PartnerReviewInsert = typeof partnerReview.$inferInsert;
export type PartnerReviewUpdate = Partial<Omit<PartnerReviewInsert, 'id' | 'partnerId' | 'userId' | 'createdAt'>>;

export type PartnerRequestRecord = typeof partnerRequest.$inferSelect;
export type PartnerRequestInsert = typeof partnerRequest.$inferInsert;
export type PartnerRequestUpdate = Partial<Omit<PartnerRequestInsert, 'id' | 'userId' | 'createdAt'>>;
